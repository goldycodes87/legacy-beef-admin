...
export default function PrintCutSheetPage() {
  ...
  const answersForHalf = (half: 'A' | 'B' | null) => {
    if (!session.dual_cut_sheet || half === null) {
      return session.cut_sheet_answers || [];
    }
    const sections = [...new Set((session.cut_sheet_answers || []).map(a => a.section))];
    const result: CutSheetAnswer[] = [];
    for (const section of sections) {
      const halfSpecific = session.cut_sheet_answers.find(
      ...
      const bothHalves = session.cut_sheet_answers.find(
  ...
  const sheetsToRender = halfParam
    ? [{ half: halfParam, answers: answersForHalf(halfParam) }]
    : session.purchase_type === 'whole' && session.dual_cut_sheet
      ? [
          { half: 'A' as const, answers: answersForHalf('A') },
          { half: 'B' as const, answers: answersForHalf('B') },
        ]
      : [{ half: null as 'A' | 'B' | null, answers: answersForHalf(null) }];

  const renderCutSheet = (answers: CutSheetAnswer[], halfLabel: 'A' | 'B' | null, index: number) => {
    const a = answers || [];
    ...
    return (
      <div key={halfLabel || 'single'} className={index > 0 ? 'page-break' : ''}>
  ...
  return (
    <div style={{fontFamily:'Arial, sans-serif', maxWidth:850, margin:'0 auto', padding:16, fontSize:12, lineHeight:1.4, WebkitPrintColorAdjust:'exact'}}>
      {sheetsToRender.map((sheet, index) => renderCutSheet(sheet.answers, sheet.half, index))}
      <style>{`
        @media print {
          body { margin: 0; }
          button { display: none; }
